...
class ChunkBuilder:
    ...
    def _regex_calls(self, content: str) -> list[str]:
        ...
        return sorted({call for call in calls if call.split(".")[-1] not in keywords})
    ...
    def _regex_references(self, content: str) -> list[str]:
        ...
        return sorted({ref for ref in refs if ref not in keywords})
    ...
    def _to_symbol(self, metadata: FileMetadata, chunk: CodeChunk, raw: RawChunk) -> CodeSymbol:
        return CodeSymbol(
            ...
            indexed_at=utcnow().astimezone(timezone.utc),
        )
    ...
    def _to_edges(self, metadata: FileMetadata, chunk: CodeChunk, raw: RawChunk) -> list[CodeEdge]:
        ...
        if metadata.is_config:
            edges.append(self._edge(metadata, chunk, source, metadata.file_path, EdgeType.CONFIGURES, 0.4))
    ...
    def _route_patterns(self, raw: RawChunk) -> list[str]:
        ...
        for decorator in raw.decorators or []:
            if any(token in decorator.lower() for token in ("route", "get", "post", "put", "delete", "patch")):
